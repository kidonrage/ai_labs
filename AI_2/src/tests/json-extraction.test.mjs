...
import { extractJsonObject as extractAgentJsonObject } from "../agent/json.js";
...
async function main() {
  ...
  assert.deepEqual(extractAgentJsonObject(JSON.stringify(direct)), direct);
  ...
  assert.deepEqual(extractAgentJsonObject(wrapped), expected);
  assert.deepEqual(extractJsonObjectFromText(wrapped), expected);

  assert.equal(extractJsonObject('["not","an","object"]'), null);
  assert.equal(extractAgentJsonObject('"plain string"'), null);

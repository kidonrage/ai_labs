...
export class Agent {
  ...
  static normalizeTaskState(value) {
    const base = Agent.makeDefaultTaskState();
  ...
  transitionTo(nextStage, expectedAction) {
    const prev = Agent.normalizeTaskState(this.taskState);
    this.taskState = Agent.normalizeTaskState({
      ...prev,
      stage: nextStage,
      expectedAction,
    ...
    this._emitStateChanged();
    return { from: prev.stage, to: this.taskState.stage };
  }
  ...
  pauseTask() {
    const state = Agent.normalizeTaskState(this.taskState);
  ...
  _buildTaskStepInput(stage, context = {}) {
    const ts = Agent.normalizeTaskState(this.taskState);
  ...
  async runTaskStep(context = {}) {
    ...
    const next = Agent.normalizeTaskState(this.taskState);
    if (stage === "planning") {
    ...
    const finalRawText = await this._runTaskLLMStep(
      this._buildTaskFinalizationInput(context, answerText),
    );
    let finalText = String(finalRawText || "").trim();
    const parsedFinal = this._extractJsonObject(finalRawText);
    if (
      parsedFinal &&
